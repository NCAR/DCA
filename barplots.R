...
## rev = invert plot (wet top vs wet bottom)
for(invert in c(TRUE, FALSE)){
    if(invert){
        ifun <- rev
        odir <- paste0(outdir, "/inverted")
    } else {
        ifun <- I
        odir <- outdir
    }
    system(paste("mkdir -p", odir))
    bmap <- lapply(bucketmap, ifun)
            ...
            for(ptype in c("change", "grouped")){
                ...
                for(GCM in gcms){
                    ...
                    pctarr <- array(dim=c(3, 1+3*nm),
                                    dimnames=list(bucket=ifun(buckets), method=NULL))
                ...
                legarr <-array(1:9, dim=c(3,3),
                               dimnames=list(names(bucketmap), ifun(buckets)))
